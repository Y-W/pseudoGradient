...
# Define PseudoGradient for ReLU
from theano.scalar.basic import UnaryScalarOp, same_out_nocomplex, sgn, maximum, exp, float_types, int_types
from theano.tensor.elemwise import Elemwise

class ReLU_PG(UnaryScalarOp):
    nfunc_spec = ('relu_pg', 1, 1)
...
def load_data():
    ...
    ys = []
    for j in range(5):
      d = unpickle('../cifar10/data_batch_'+`j+1`)
      x = d['data']
      y = d['labels']
      xs.append(x)
      ys.append(y)
...
def build_cnn(input_var=None, n=5):
    ...
    def residual_block(l, increase_dim=False):
        ...
        stack_1 = batch_norm(ConvLayer(l, num_filters=out_num_filters, filter_size=(3,3), stride=first_stride, nonlinearity=relu_pg, pad='same', W=lasagne.init.HeNormal(gain='relu'), flip_filters=False))
    ...
    l = batch_norm(ConvLayer(l_in, num_filters=16, filter_size=(3,3), stride=(1,1), nonlinearity=relu_pg, pad='same', W=lasagne.init.HeNormal(gain='relu'), flip_filters=False))
...
def main(n=5, num_epochs=100, model=None):
    ...
    if model is None:
        ...
        updates = lasagne.updates.momentum(
                loss, params, learning_rate=sh_lr, momentum=0.5)
